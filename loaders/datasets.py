...
class KineticsDataset(BaseDataset):
	...
	def get_clips(self):
		...
		indices_filter = [i for i, vid_id in enumerate(vid_ids) if vid_id in vid_info and not vid_info[vid_id]["error"] and vid_info[vid_id]["n_frames"] > 10]
...
class SomethingSomething(BaseDataset):
	...
	def get_clips(self):
		...
		videos = glob.glob(os.path.join(self.config.data_base_path, "something/vid/*/*.webm"))
		vid_ids = [fn.split('/')[-1][:-5] for fn in videos]

		with open(os.path.join(self.config.data_base_path, "something/meta/vid_info.pkl"), "rb") as f:
			vid_info = pickle.load(f)
